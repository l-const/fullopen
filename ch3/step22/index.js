...
app.post("/api/persons", (request, response, next) => {
    ...
    Person.find( {$and : [ { name: request.body.name } , { number:  request.body.number } ]})
          .then(dupQuery => {
                console.log(dupQuery)
                console.log(dupQuery, dupQuery.length)
                if(dupQuery.length !== 0){
                    return response.status(400).send({error: "DuplicateError!"})
                }
